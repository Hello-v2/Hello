...
class AdminExamController {
    ...
    public function store() {
        ...
        if ($conn) {
            try {
                $stmt = $conn->prepare("
                    INSERT INTO exams (title, slug, description, duration_minutes, passing_score, is_published, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
                ");
    ...
    public function update($id) {
        ...
        if ($conn) {
            try {
                ...
                $stmt = $conn->prepare("
                    UPDATE exams
                    SET title = ?, slug = ?, description = ?, duration_minutes = ?, passing_score = ?, is_published = ?, course_id = NULL, updated_at = NOW()
                    WHERE id = ?
                ");
